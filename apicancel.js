...
export default async function handler(req, res) {
  ...
  if (!customer_uid) {
    return res.status(400).json({ success: false, message: 'customer_uid가 누락되었습니다.' });
  }
  ...
  try {
    ...
    // 2. 포트원 예약 결제 스케줄 취소 (customer_uid의 모든 미래 결제 예약 삭제)
    ...
    return res.status(200).json({
      success: true,
      message: '예약된 정기결제가 포트원에서 안전하게 취소되었습니다.',
